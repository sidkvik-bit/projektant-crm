...
export function QuoteItemsPanel({
  ...
  items: QuoteItem[];
  vatRate: number;
  onAdd: (quoteId: string, name: string, quantity: number, unit: string, unitPrice: number) => Promise<void>;
  onUpdate: (
    quoteId: string,
    itemId: string,
    name: string,
  ...
  onDelete: (quoteId: string, itemId: string) => Promise<void>;
}) {
  ...
  async function handleAddSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      await onAdd(quoteId, draft.name, Number(draft.quantity) || 0, draft.unit, Number(draft.unitPrice) || 0);
